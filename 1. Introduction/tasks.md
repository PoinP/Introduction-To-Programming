...
int main()
{
	...
	std::cout << "Please enter e 3 digit number: ";
	...
	// Решава се и без цикъл като се
	// копира и постави 3 пъти долния фрагмент код
	for (int i = 0; i < 3; i++)
	{
		sevenCount += userInput % 10 == 7;
		userInput /= 10;
	}

	std::cout << "The count of sevens is: " << sevenCount << std::endl;
}
...
int main()
{
	...
	double max = firstNumber * (firstNumber >= secondNumber) + secondNumber * (secondNumber > firstNumber);

	std::cout << "The max number between the two is: " << max;
}
